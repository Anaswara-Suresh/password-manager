...
void PasswordList::loadPasswords()
{
    ...
    for (const QList<QVariant> &record : all) {
        ...
        QString sitePlain = Crypto::decrypt(siteCipher);
        QString userPlain = Crypto::decrypt(userCipher);
        QString passPlain = Crypto::decrypt(passCipher);
...
void PasswordList::onEditButtonClicked()
{
    ...
    QString sitePlain = Crypto::decrypt(query.value("site").toByteArray());
    QString userPlain = Crypto::decrypt(query.value("username").toByteArray());
    QString passPlain = Crypto::decrypt(query.value("password").toByteArray());
    ...
    QByteArray siteCipher = newSite.isEmpty() ? query.value("site").toByteArray() : Crypto::encrypt(newSite);
    QByteArray userCipher = newUser.isEmpty() ? query.value("username").toByteArray() : Crypto::encrypt(newUser);
    QByteArray passCipher = newPass.isEmpty() ? query.value("password").toByteArray() : Crypto::encrypt(newPass);
